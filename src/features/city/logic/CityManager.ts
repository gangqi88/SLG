...
export class CityManager {
  ...
  public upgradeBuilding(buildingId: string): boolean {
    ...
    // Check resources
    if (
      this.resourceManager.getResource(ResourceType.WOOD) >= cost &&
      this.resourceManager.getResource(ResourceType.STONE) >= cost
    ) {
      if (
        this.resourceManager.consumeResource(ResourceType.WOOD, cost) &&
  ...
  private recalculateProduction() {
    ...
    this.resourceManager.setProduction(
      ResourceType.WOOD,
      baseProduction[ResourceType.WOOD] * (1 + woodBonus),
    );
    this.resourceManager.setProduction(
      ResourceType.STONE,
      baseProduction[ResourceType.STONE] * (1 + stoneBonus),
    );
    this.resourceManager.setProduction(
      ResourceType.FOOD,
      baseProduction[ResourceType.FOOD] * (1 + foodBonus),
    );
    this.resourceManager.setProduction(
      ResourceType.COIN,
      baseProduction[ResourceType.COIN] * (1 + coinBonus),
    );
  }
