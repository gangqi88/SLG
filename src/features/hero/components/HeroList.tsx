...
const HeroList: React.FC = () => {
  ...
  const filteredHeroes = filterRace === 'All' 
    ? allHeroes 
    : allHeroes.filter(h => h.race === filterRace);
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <button onClick={() => setFilterRace('All')} style={{ marginRight: '10px' }}>全部</button>
        <button onClick={() => setFilterRace(Race.HUMAN)} style={{ marginRight: '10px' }}>人族</button>
        <button onClick={() => setFilterRace(Race.ANGEL)} style={{ marginRight: '10px' }}>天使</button>
      ...
      <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' }}>
        {filteredHeroes.map(hero => (
          ...
          >
            ...
            <p style={{ margin: '5px 0', fontSize: '0.9em', color: '#aaa' }}>{hero.race} | {hero.position}</p>
