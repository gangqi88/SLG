    ...
    >
      ...
      >
        ...
        >
          <h2
            style={{
              color: getQualityColor(hero.quality),
              margin: 0,
              display: 'flex',
              alignItems: 'center',
              gap: '10px',
          ...
          >
            ...
            <span
              style={{
                fontSize: '0.5em',
                color: '#fff',
                backgroundColor: getQualityColor(hero.quality),
                padding: '2px 6px',
                borderRadius: '4px',
              }}
            >
        ...
        >
          <div>
            <p style={{ margin: '5px 0' }}>
              <strong style={{ color: '#aaa' }}>种族:</strong>{' '}
              <span style={{ color: '#fff' }}>{hero.race}</span>
            </p>
            <p style={{ margin: '5px 0' }}>
              <strong style={{ color: '#aaa' }}>定位:</strong>{' '}
              <span style={{ color: '#fff' }}>{hero.position}</span>
            </p>
            <p style={{ margin: '5px 0' }}>
              <strong style={{ color: '#aaa' }}>兵种:</strong>{' '}
              <span style={{ color: '#fff' }}>{hero.troopType}</span>
            </p>
          </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ marginBottom: '10px' }}>
            <strong>主动: {hero.activeSkill.name}</strong>{' '}
            {hero.activeSkill.cooldown && <small>(CD: {hero.activeSkill.cooldown}s)</small>}
            <p style={{ margin: '5px 0', fontSize: '0.9em', color: '#aaa' }}>
              {hero.activeSkill.description}
            </p>
          </div>
          <div style={{ marginBottom: '10px' }}>
            <strong>被动: {hero.passiveSkill.name}</strong>
            <p style={{ margin: '5px 0', fontSize: '0.9em', color: '#aaa' }}>
              {hero.passiveSkill.description}
            </p>
          </div>
        </div>
        ...
        {hero.bond && (
          <div style={{ marginBottom: '20px' }}>
            ...
            <p style={{ margin: '5px 0', fontSize: '0.9em', color: '#aaa' }}>
              {hero.bond.description}
            </p>
            <p style={{ margin: '5px 0', fontSize: '0.9em', color: '#8c8' }}>
              效果: {hero.bond.effect}
            </p>
          </div>
        )}
