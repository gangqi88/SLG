...
describe('BuildingNFT', function () {
    ...
    describe('铸造建筑', function () {
        it('应该成功铸造城堡', async function () {
            const tx = await buildingNFT.mintBuilding(
                owner.address,
                CASTLE,
                1,
                30
    ...
    describe('升级建筑', function () {
        ...
        it('不应该升级到最大等级', async function () {
            ...
            await expect(
                buildingNFT.upgradeBuilding(2)
            ).to.be.revertedWith('Already at max level');
        });

        it('非拥有者不应该升级', async function () {
            await expect(
                buildingNFT.connect(addr1).upgradeBuilding(1)
    ...
    describe('建筑转移', function () {
        ...
        it('不应该非法转移', async function () {
            await expect(
                buildingNFT.connect(addr2).transferFrom(owner.address, addr2.address, 1)
            ).to.be.reverted;
        });
    });
    ...
    describe('事件测试', function () {
        it('应该触发BuildingMinted事件', async function () {
            ...
            const event = receipt.logs.find(log => 
                log.fragment && log.fragment.name === 'BuildingMinted'
            );
        ...
        it('应该触发BuildingUpgraded事件', async function () {
            ...
            const event = receipt.logs.find(log => 
                log.fragment && log.fragment.name === 'BuildingUpgraded'
            );
        ...
        it('应该触发BuildingTransferred事件', async function () {
            ...
            const event = receipt.logs.find(log => 
                log.fragment && log.fragment.name === 'BuildingTransferred'
            );
    ...
    describe('权限控制', function () {
        it('仅MINTER_ROLE可以铸造', async function () {
            await expect(
                buildingNFT.connect(addr1).mintBuilding(addr1.address, CASTLE, 1, 30)
            ).to.be.reverted;
        });
