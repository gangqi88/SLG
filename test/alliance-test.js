...
describe('AllianceNFT', function () {
    ...
    describe('加入联盟', function () {
        ...
        it('不应该加入不存在的联盟', async function () {
            await expect(
                allianceNFT.connect(addr1).joinAlliance(999)
        ...
        it('不应该重复加入联盟', async function () {
            ...
            await expect(
                allianceNFT.connect(addr1).joinAlliance(1)
            ).to.be.revertedWith('Already in an alliance');
        });
        ...
        it('不应该加入满员联盟', async function () {
            ...
            await expect(
                allianceNFT.connect(randomWallet).joinAlliance(1)
            ).to.be.revertedWith('Alliance is full');
        });
    });
    ...
    describe('离开联盟', function () {
        ...
        it('不应该离开不在联盟中的地址', async function () {
            await expect(
                allianceNFT.connect(addr2).leaveAlliance()
            ).to.be.revertedWith('Not in any alliance');
        });
    });
    ...
    describe('贡献系统', function () {
        ...
        it('不应该存款0', async function () {
            await expect(
                allianceNFT.contribute(1, { value: 0 })
    ...
    describe('升级系统', function () {
        ...
        it('不应该超过最大等级', async function () {
            ...
            await expect(
                allianceNFT.upgradeLevel(1)
            ).to.be.revertedWith('Already at max level');
        });
    });
    ...
    describe('事件测试', function () {
        it('应该触发AllianceCreated事件', async function () {
            ...
            const event = receipt.logs.find(log => 
                log.fragment && log.fragment.name === 'AllianceCreated'
            );
        ...
        it('应该触发MemberJoined事件', async function () {
            ...
            const event = receipt.logs.find(log => 
                log.fragment && log.fragment.name === 'MemberJoined'
            );
